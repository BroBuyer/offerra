/**
 * Copy an English template tree into langs/ms and apply EN→Malay replacements.
 *
 * Usage:
 *   node scripts/build-ms-from-en.mjs noctra
 *   node scripts/build-ms-from-en.mjs default
 *   node scripts/build-ms-from-en.mjs multilang
 *   node scripts/build-ms-from-en.mjs aurel
 */
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import thaloraMs from './thalora-i18n/ms.mjs';
...
const TEMPLATES = {
  noctra: { src: 'templates/noctra', dest: 'templates/noctra/langs/ms' },
  default: { src: 'templates/default', dest: 'templates/default/langs/ms' },
  multilang: { src: 'templates/multilang', dest: 'templates/multilang/langs/ms' },
  aurel: { src: 'templates/aurel/langs/en', dest: 'templates/aurel/langs/ms' },
};
...
function build(id) {
  ...
  const templateRoot = path.join(ROOT, 'templates', id === 'aurel' ? 'aurel' : id);
