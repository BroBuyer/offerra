/**
 * Build Latvian (lv) packs for every Offerra template.
 * Usage: node scripts/add-lv-lang.mjs
 */
import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';
import { spawnSync } from 'child_process';
import extra from './lv-i18n/extra-en-lv.mjs';
...
function prepareLumenSkeleton() {
  ...
  const dest = path.join(src, 'langs', 'lv');
...
run('scripts/build-lumen-langs.mjs', 'lv');
patchConfig(path.join(ROOT, 'templates', 'lumen', 'langs', 'lv', 'includes', 'config.php'));
applyExtra(path.join(ROOT, 'templates', 'lumen', 'langs', 'lv'));
run('scripts/build-recupero-langs.mjs', 'lv');
copyFile(
  path.join(ROOT, 'templates', 'recupero', 'includes', 'helpers.php'),
  path.join(ROOT, 'templates', 'recupero', 'langs', 'lv', 'includes', 'helpers.php'),
);
run('scripts/build-velora-langs.mjs', 'lv');
patchConfig(path.join(ROOT, 'templates', 'velora', 'langs', 'lv', 'includes', 'config.php'));
run('scripts/build-cetra-langs.mjs', 'lv');
copyFile(
  path.join(ROOT, 'templates', 'cetra', 'includes', 'helpers.php'),
  path.join(ROOT, 'templates', 'cetra', 'langs', 'lv', 'includes', 'helpers.php'),
);
run('scripts/build-thalora-langs.mjs', 'lv');
run('scripts/build-aurel-from-en.mjs', 'lv');
run('scripts/build-lv-from-en.mjs');
