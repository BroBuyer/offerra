import { useState } from 'react';

function EyeIcon() {
    return (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
            <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />
            <circle cx="12" cy="12" r="3" />
        </svg>
    );
}

function EyeOffIcon() {
    return (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
            <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94" />
            <path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19" />
            <path d="M1 1l22 22" />
            <path d="M14.12 14.12a3 3 0 1 1-4.24-4.24" />
        </svg>
    );
}

export default function UserCredentialsCard({ user, title, onDismiss }) {
    const [visible, setVisible] = useState(false);
    const [copied, setCopied] = useState(false);

    const copyCredentials = async () => {
        const text = `Offerra\nEmail: ${user.email}\nПароль: ${user.password}`;

        try {
            await navigator.clipboard.writeText(text);
            setCopied(true);
            setTimeout(() => setCopied(false), 2000);
        } catch {
            // ignore
        }
    };

    return (
        <section className="card credentials-card">
            <h3>{title}</h3>
            <p className="card-desc">
                Пароль показується один раз. Збережіть або скопіюйте — пізніше переглянути не вийде.
            </p>
            <dl className="credentials-list">
                <div className="credentials-row">
                    <dt>Імʼя</dt>
                    <dd>{user.name}</dd>
                </div>
                <div className="credentials-row">
                    <dt>Email</dt>
                    <dd><code>{user.email}</code></dd>
                </div>
                <div className="credentials-row">
                    <dt>Пароль</dt>
                    <dd>
                        <div className="credentials-password">
                            <code className="credentials-value">
                                {visible ? user.password : '••••••••••••'}
                            </code>
                            <button
                                type="button"
                                className="secret-input-toggle credentials-toggle"
                                onClick={() => setVisible((v) => !v)}
                                aria-label={visible ? 'Приховати пароль' : 'Показати пароль'}
                            >
                                {visible ? <EyeOffIcon /> : <EyeIcon />}
                            </button>
                        </div>
                    </dd>
                </div>
            </dl>
            <div className="btn-row">
                <button type="button" className="btn btn-primary" onClick={copyCredentials}>
                    {copied ? 'Скопійовано' : 'Копіювати дані'}
                </button>
                {onDismiss && (
                    <button type="button" className="btn btn-ghost" onClick={onDismiss}>
                        Закрити
                    </button>
                )}
            </div>
        </section>
    );
}
