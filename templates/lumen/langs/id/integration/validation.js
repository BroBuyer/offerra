...
const phoneErrorMap = {
  0: 'Masukkan nomor telepon yang valid',
  1: 'Kode negara tidak valid',
  2: 'Nomor telepon terlalu pendek',
  3: 'Nomor telepon terlalu panjang',
  4: 'Masukkan nomor telepon yang valid',
};
...
function validateTelepon(phoneInput, iti) {
  ...
  if (!trimmed) return 'Masukkan nomor telepon Anda';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Masukkan nomor telepon yang valid';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Sesi berakhir. Muat ulang halaman dan coba lagi.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Terjadi kesalahan. Silakan coba lagi nanti.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Kesalahan koneksi. Periksa koneksi internet Anda dan coba lagi.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
