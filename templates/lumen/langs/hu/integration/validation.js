...
const phoneErrorMap = {
  0: 'Adjon meg egy érvényes telefonszámot',
  1: 'Érvénytelen országkód',
  2: 'A telefonszám túl rövid',
  3: 'A telefonszám túl hosszú',
  4: 'Adjon meg egy érvényes telefonszámot',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Adja meg a telefonszámát';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Adjon meg egy érvényes telefonszámot';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'A munkamenet lejárt. Töltse újra az oldalt, és próbálja újra.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Hiba történt. Kérjük, próbálja meg később.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Kapcsolódási hiba. Ellenőrizze az internetkapcsolatot, és próbálja újra.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
