...
const phoneErrorMap = {
  0: 'Įveskite galiojantį telefono numerį',
  1: 'Netinkamas šalies kodas',
  2: 'Telefono numeris per trumpas',
  3: 'Telefono numeris per ilgas',
  4: 'Įveskite galiojantį telefono numerį',
};
...
function validateTelefonas(phoneInput, iti) {
  ...
  if (!trimmed) return 'Įveskite savo telefono numerį';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Įveskite galiojantį telefono numerį';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Sesija baigėsi. Įkelkite puslapį iš naujo ir bandykite dar kartą.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Kažkas nutiko. Bandykite dar kartą vėliau.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Ryšio klaida. Patikrinkite interneto ryšį ir bandykite dar kartą.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
