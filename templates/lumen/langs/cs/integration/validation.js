...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Relace vypršela. Obnovte stránku a zkuste to znovu.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Došlo k chybě. Zkuste to prosím později.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Chyba připojení. Zkontrolujte internetové připojení a zkuste to znovu.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
