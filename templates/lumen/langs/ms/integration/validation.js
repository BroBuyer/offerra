...
const phoneErrorMap = {
  0: 'Masukkan nombor telefon yang sah',
  1: 'Kod negara tidak sah',
  2: 'Nombor telefon terlalu pendek',
  3: 'Nombor telefon terlalu panjang',
  4: 'Masukkan nombor telefon yang sah',
};
...
function validateTelefon(phoneInput, iti) {
  ...
  if (!trimmed) return 'Masukkan nombor telefon anda';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Masukkan nombor telefon yang sah';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Sesi tamat. Muat semula halaman dan cuba lagi.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Sesuatu tidak kena. Sila cuba lagi kemudian.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Ralat sambungan. Semak sambungan internet anda dan cuba lagi.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
