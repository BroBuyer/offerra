...
const phoneErrorMap = {
  0: 'Εισαγάγετε έγκυρο αριθμό τηλεφώνου',
  1: 'Μη έγκυρος κωδικός χώρας',
  2: 'Ο αριθμός τηλεφώνου είναι πολύ σύντομος',
  3: 'Ο αριθμός τηλεφώνου είναι πολύ μακρύς',
  4: 'Εισαγάγετε έγκυρο αριθμό τηλεφώνου',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Εισαγάγετε τον αριθμό τηλεφώνου σας';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Εισαγάγετε έγκυρο αριθμό τηλεφώνου';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Η συνεδρία έληξε. Φορτώστε ξανά τη σελίδα και δοκιμάστε ξανά.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Κάτι πήγε στραβά. Δοκιμάστε ξανά αργότερα.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Σφάλμα σύνδεσης. Ελέγξτε τη σύνδεσή σας στο διαδίκτυο και δοκιμάστε ξανά.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
