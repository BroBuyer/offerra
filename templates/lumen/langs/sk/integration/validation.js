...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Relácia vypršala. Obnovte stránku a skúste to znova.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Došlo k chybe. Skúste to prosím neskôr.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Chyba pripojenia. Skontrolujte internetové pripojenie a skúste to znova.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
