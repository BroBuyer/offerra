...
const phoneErrorMap = {
  0: 'Introduceți un număr de telefon valid',
  1: 'Prefix național invalid',
  2: 'Numărul de telefon este prea scurt',
  3: 'Numărul de telefon este prea lung',
  4: 'Introduceți un număr de telefon valid',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!iti.isValidNumber()) {
    ...
    return phoneErrorMap[code] || 'Introduceți un număr de telefon valid';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Sesiunea a expirat. Reîncarcă pagina și încearcă din nou.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'A apărut o eroare. Te rugăm să încerci din nou mai târziu.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Eroare de conexiune. Verifică conexiunea la internet și încearcă din nou.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
