...
const phoneErrorMap = {
  0: 'Lütfen geçerli bir telefon numarası girin',
  1: 'Geçersiz ülke kodu',
  2: 'Telefon numarası çok kısa',
  3: 'Telefon numarası çok uzun',
  4: 'Lütfen geçerli bir telefon numarası girin',
};
...
/** Ad, soyad, e-posta — tarayıcı doğal doğrulaması. */
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Lütfen telefon numaranızı girin';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Lütfen geçerli bir telefon numarası girin';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Oturum süresi doldu. Lütfen sayfayı yenileyip tekrar deneyin.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Bir hata oluştu. Lütfen daha sonra tekrar deneyin.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Bağlantı hatası. Lütfen internet bağlantınızı kontrol edip tekrar deneyin.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
