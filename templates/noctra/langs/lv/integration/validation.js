...
const phoneErrorMap = {
  0: 'Ievadiet derīgu tālruņa numuru',
  1: 'Nederīgs valsts kods',
  2: 'Tālruņa numurs ir pārāk īss',
  3: 'Tālruņa numurs ir pārāk garš',
  4: 'Ievadiet derīgu tālruņa numuru',
};
...
function validateTālrunis(phoneInput, iti) {
  const trimmed = phoneInput.value.trim();
  if (!trimmed) return 'Ievadiet tālruņa numuru';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Ievadiet derīgu tālruņa numuru';
  }
  return '';
}
...
async function resolveVisitorTālrunisCountry(form, onlyCountries) {
  try {
...
function syncTālrunisCountryHidden(form, iti) {
...
function setupFormValidation(form) {
  ...
  if (singleCountry) {
    ...
    resolveVisitorTālrunisCountry(form, onlyCountries).then((code) => {
      if (code && code !== phoneCountry) {
        iti.setCountry(code);
        syncTālrunisCountryHidden(form, iti);
      }
    });
  }

  phone.addEventListener('countrychange', () => syncTālrunisCountryHidden(form, iti));
  ...
  phone.addEventListener('blur', () => {
    ...
    const msg = validateTālrunis(phone, iti);
  ...
  form.addEventListener('submit', async (e) => {
    ...
    clearFieldError(phone);
    const phoneError = validateTālrunis(phone, iti);
    if (phoneError) {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Sesija beigusies. Pārlādējiet lapu un mēģiniet vēlreiz.');
      preloader?.classList.add('hidden');
      return;
    }

    syncTālrunisCountryHidden(form, iti);
    const fullTālrunis = iti.getNumber();
    const formData = new FormData(form);
    formData.set('fullphone', fullTālrunis);
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Kaut kas nogāja greizi. Lūdzu, mēģiniet vēlāk.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Savienojuma kļūda. Pārbaudiet interneta savienojumu un mēģiniet vēlreiz.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
