...
async function resolveVisitor電話Country(form, onlyCountries) {
  try {
...
function sync電話CountryHidden(form, iti) {
...
function setupFormValidation(form) {
  ...
  if (singleCountry) {
    ...
    resolveVisitor電話Country(form, onlyCountries).then((code) => {
      if (code && code !== phoneCountry) {
        iti.setCountry(code);
        sync電話CountryHidden(form, iti);
      }
    });
  }

  phone.addEventListener('countrychange', () => sync電話CountryHidden(form, iti));
  ...
  phone.addEventListener('blur', () => {
    ...
    const msg = validate電話(phone, iti);
  ...
  form.addEventListener('submit', async (e) => {
    ...
    clearFieldError(phone);
    const phoneError = validate電話(phone, iti);
    if (phoneError) {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'セッションの有効期限が切れました。ページを再読み込みして再度お試しください。');
      preloader?.classList.add('hidden');
      return;
    }

    sync電話CountryHidden(form, iti);
    const full電話 = iti.getNumber();
    const formData = new FormData(form);
    formData.set('fullphone', full電話);
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || '問題が発生しました。しばらくしてから再度お試しください。');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, '接続エラーです。インターネット接続を確認して再度お試しください。');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
