...
const phoneErrorMap = {
  0: 'Ange ett giltigt telefonnummer',
  1: 'Ogiltig landskod',
  2: 'Telefonnumret är för kort',
  3: 'Telefonnumret är för långt',
  4: 'Ange ett giltigt telefonnummer',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Ange ditt telefonnummer';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Ange ett giltigt telefonnummer';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Sessionen har gått ut. Ladda om sidan och försök igen.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Något gick fel. Försök igen senare.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Anslutningsfel. Kontrollera din internetanslutning och försök igen.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
