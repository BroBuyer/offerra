...
const phoneErrorMap = {
  0: 'Voer een geldig telefoonnummer in',
  1: 'Ongeldige landcode',
  2: 'Het telefoonnummer is te kort',
  3: 'Het telefoonnummer is te lang',
  4: 'Voer een geldig telefoonnummer in',
};
...
/** Voornaam, achternaam, e-mail — native browservalidatie. */
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Vul uw telefoonnummer in';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Voer een geldig telefoonnummer in';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Sessie verlopen. Laad de pagina opnieuw en probeer het opnieuw.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Er is een fout opgetreden. Probeer het later opnieuw.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Verbindingsfout. Controleer uw internetverbinding en probeer opnieuw.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
