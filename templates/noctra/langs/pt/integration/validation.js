...
const phoneErrorMap = {
  0: 'Introduza um número de telefone válido',
  1: 'Indicativo de país inválido',
  2: 'O número de telefone é demasiado curto',
  3: 'O número de telefone é demasiado longo',
  4: 'Introduza um número de telefone válido',
};
...
/** Nome, apelido, e-mail — validação nativa do browser. */
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Introduza o seu número de telefone';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Introduza um número de telefone válido';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Sessão expirada. Recarregue a página e tente novamente.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Algo correu mal. Tente novamente mais tarde.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Erro de ligação. Verifique a sua ligação à Internet e tente novamente.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
