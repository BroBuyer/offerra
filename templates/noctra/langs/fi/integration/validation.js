...
const phoneErrorMap = {
  0: 'Syötä kelvollinen puhelinnumero',
  1: 'Virheellinen maakoodi',
  2: 'Puhelinnumero on liian lyhyt',
  3: 'Puhelinnumero on liian pitkä',
  4: 'Syötä kelvollinen puhelinnumero',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Syötä puhelinnumerosi';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Syötä kelvollinen puhelinnumero';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Istunto vanhentui. Lataa sivu uudelleen ja yritä uudelleen.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Jokin meni pieleen. Yritä myöhemmin uudelleen.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Yhteysvirhe. Tarkista internetyhteytesi ja yritä uudelleen.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
