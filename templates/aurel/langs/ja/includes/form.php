...
$form_submit = $form_submit ?? '今すぐ始める';
...
>
  <div class="form-already-registered hidden" data-already-registered>
    ...
    <p class="form-already-registered__text">
      お客様のリクエスト： <?= e(SITE_NAME) ?> を受け付けました。アドバイザーからのお電話をお待ちください。
    </p>
  </div>
...
<?php if ($is_stack): ?>
    <div class="xdzqh">
      <div class="wxts7 field" data-field="fname">
        <label class="field-label" for="<?= e($form_id) ?>-fname">名</label>
        <input class="wnzxg input" id="<?= e($form_id) ?>-fname" type="text" name="fname" placeholder="名" required pattern="[^\d]+" autocomplete="given-name">
      </div>
      <div class="wxts7 field" data-field="lname">
        <label class="field-label" for="<?= e($form_id) ?>-lname">姓</label>
        <input class="wnzxg input" id="<?= e($form_id) ?>-lname" type="text" name="lname" placeholder="姓" required pattern="[^\d]+" autocomplete="family-name">
      </div>
    </div>
    <div class="wxts7 field" data-field="email">
      <label class="field-label" for="<?= e($form_id) ?>-email">メール</label>
      <input class="wnzxg input" id="<?= e($form_id) ?>-email" type="email" name="email" placeholder="メール" required autocomplete="email" inputmode="email">
    </div>
    <div class="wxts7 field" data-field="phone">
      <label class="field-label" for="<?= e($form_id) ?>-phone">電話</label>
...
<?php endif; ?>
    ...
    <div class="nmpf6" aria-label="ご利用可能な支払い方法">
      ...
      <svg class="ajr3je" role="img" aria-label="銀行振込" viewBox="0 0 512 512"><use href="#i-building-columns"/></svg>
      <svg class="ajr3je kw4ijl" role="img" aria-label="安全" viewBox="0 0 512 512"><use href="#i-shield-halved"/></svg>
    </div>
    ...
    <p class="ac7ftax">
      続行することにより、プライバシーポリシーと利用規約に同意したものとみなします。
