...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || '電話番号を入力してください';
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'セッションの有効期限が切れました。ページを再読み込みして、もう一度お試しください。',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || '問題が発生しました。しばらくしてからもう一度お試しください。',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          '接続エラーです。インターネット接続を確認して、もう一度お試しください。',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
