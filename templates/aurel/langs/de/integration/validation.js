...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Bitte geben Sie Ihre Telefonnummer ein';
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Sitzung abgelaufen. Bitte laden Sie die Seite neu und versuchen Sie es erneut.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Etwas ist schiefgelaufen. Bitte versuchen Sie es später erneut.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Verbindungsfehler. Prüfen Sie Ihre Internetverbindung und versuchen Sie es erneut.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
