...
$form_submit = $form_submit ?? 'Pradėkite dabar';
...
>
  <div class="form-already-registered hidden" data-already-registered>
    ...
    <p class="form-already-registered__title">Jūs jau užsiregistravote</p>
    <p class="form-already-registered__text">
      Jūsų prašymas su<?= e(SITE_NAME) ?>buvo gauta. Prašome laukti mūsų konsultanto skambučio.
    </p>
  </div>
...
<?php if ($is_stack): ?>
    <div class="xdzqh">
      <div class="wxts7 field" data-field="fname">
        <label class="field-label" for="<?= e($form_id) ?>-fname">Vardas</label>
        <input class="wnzxg input" id="<?= e($form_id) ?>-fname" type="text" name="fname" placeholder="Vardas" required pattern="[^\d]+" autocomplete="given-name">
      </div>
      <div class="wxts7 field" data-field="lname">
        <label class="field-label" for="<?= e($form_id) ?>-lname">Pavardė</label>
        <input class="wnzxg input" id="<?= e($form_id) ?>-lname" type="text" name="lname" placeholder="Pavardė" required pattern="[^\d]+" autocomplete="family-name">
      </div>
    </div>
    <div class="wxts7 field" data-field="email">
      <label class="field-label" for="<?= e($form_id) ?>-email">El. paštas</label>
      <input class="wnzxg input" id="<?= e($form_id) ?>-email" type="email" name="email" placeholder="El. paštas" required autocomplete="email" inputmode="email">
    </div>
    <div class="wxts7 field" data-field="phone">
      <label class="field-label" for="<?= e($form_id) ?>-phone">Telefonas</label>
      <input class="wnzxg input" id="<?= e($form_id) ?>-phone" type="tel" name="fullphone" placeholder="7123 456789" autocomplete="tel" inputmode="tel">
...
<?php endif; ?>
    ...
    <div class="nmpf6" aria-label="Priimti mokėjimo būdai">
      ...
      <svg class="ajr3je" role="img" aria-label="Banko pavedimas" viewBox="0 0 512 512"><use href="#i-building-columns"/></svg>
      <svg class="ajr3je kw4ijl" role="img" aria-label="Saugus" viewBox="0 0 512 512"><use href="#i-shield-halved"/></svg>
    </div>
    ...
    <p class="ac7ftax">
      Tęsdami sutinkate su mūsų privatumo politika ir naudojimo sąlygomis.
