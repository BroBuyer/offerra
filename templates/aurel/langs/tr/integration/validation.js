...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Lütfen telefon numaranızı girin';
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Oturum süresi doldu. Lütfen sayfayı yenileyip tekrar deneyin.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Bir şeyler ters gitti. Lütfen daha sonra tekrar deneyin.',
        );
        return;
      }
