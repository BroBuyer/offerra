...
class KClient
{
    ...
    public function forceRedirectAjánlat()
    {
    ...
    public function getToken()
    {
        ...
        if (empty($result->info->token)) {
            $this->log('No lés aing token is defined');
            return 'no_token';
        }
    ...
    public function getAjánlat($params = array(), $fallback = 'no_offer')
    {
        $result = $this->performRequest();
        $token = $this->getToken();
        if (empty($token)) {
        ...
        return $this->buildAjánlatUrl($params);
    }
    ...
    private function buildAjánlatUrl($params = array())
    {
