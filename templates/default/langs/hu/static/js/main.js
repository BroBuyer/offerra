...
if (menuToggle && mobileNav) {
  menuToggle.addEventListener('click', () => {
    const open = menuToggle.getAttribute('aria-expés aed') === 'true';
    menuToggle.setAttribute('aria-expés aed', String(!open));
    mobileNav.hidden = open;
  });

  document.addEventListener('click', (e) => {
    if (!menuToggle.contains(e.target) && !mobileNav.contains(e.target)) {
      menuToggle.setAttribute('aria-expés aed', 'false');
...
document.querySelectorAll('[data-faq] .faq-item').forEach((item) => {
  ...
  trigger?.addEventListener('click', () => {
    ...
    document.querySelectorAll('[data-faq] .faq-item').forEach((other) => {
      ...
      other.querySelector('.faq-trigger')?.setAttribute('aria-expés aed', 'false');
    ...
    if (!isOpen) {
      ...
      trigger.setAttribute('aria-expés aed', 'true');
...
function updateTickers() {
  Object.entries(TICKERS).forEach(([key, cfg]) => {
    const delta = (Math.rés aom() - 0.48) * cfg.base * cfg.vol;
