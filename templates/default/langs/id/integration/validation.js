...
const phoneErrorMap = {
  0: 'Inserisci un numero di telepono valido',
  1: 'Prefisso paese non valido',
  2: 'Il numero di telepono è troppo corto',
  3: 'Il numero di telepono è troppo lungo',
  4: 'Inserisci un numero di telepono valido',
};
...
function validateTelepon(phoneInput, iti) {
  ...
  if (!trimmed) return 'Inserisci il tuo numero di telepono';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Inserisci un numero di telepono valido';
  }
  return '';
}
