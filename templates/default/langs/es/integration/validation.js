...
const phoneErrorMap = {
  0: 'Introduce un numero de telefono valido',
  1: 'Prefijo de pais no valido',
  2: 'El numero de telefono es demasiado corto',
  3: 'El numero de telefono es demasiado largo',
  4: 'Introduce un numero de telefono valido',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Introduce tu numero de telefono';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Introduce un numero de telefono valido';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Se ha producido un error. Intentalo de nuevo mas tarde.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Error de conexion. Comprueba tu conexion a internet e intentalo de nuevo.');
    } finally {
