...
$form_submit = $form_submit ?? 'Nu beginnen';
...
>
  <div class="form-already-registered hidden" data-already-registered>
    ...
    <p class="form-already-registered__title">U bent al geregistreerd</p>
    <p class="form-already-registered__text">
      Uw aanvraag bij <?= e(SITE_NAME) ?> is ontvangen. Wacht op het telefoontje van onze adviseur.
    </p>
  </div>
...
<?php if ($is_stack): ?>
    <div class="rtjqp">
      <div class="jn406 field" data-field="fname">
        <label class="field-label" for="<?= e($form_id) ?>-fname">Naam</label>
        <input class="cm9aayi input" id="<?= e($form_id) ?>-fname" type="text" name="fname" placeholder="Naam" required pattern="[^\d]+" autocomplete="given-name">
      </div>
      <div class="jn406 field" data-field="lname">
        <label class="field-label" for="<?= e($form_id) ?>-lname">Achternaam</label>
        <input class="cm9aayi input" id="<?= e($form_id) ?>-lname" type="text" name="lname" placeholder="Achternaam" required pattern="[^\d]+" autocomplete="family-name">
      </div>
    </div>
    <div class="jn406 field" data-field="email">
      <label class="field-label" for="<?= e($form_id) ?>-email">E-mailadres</label>
      <input class="cm9aayi input" id="<?= e($form_id) ?>-email" type="email" name="email" placeholder="E-mailadres" required autocomplete="email" inputmode="email">
    </div>
    <div class="jn406 field" data-field="phone">
      <label class="field-label" for="<?= e($form_id) ?>-phone">Telefoon</label>
      <input class="cm9aayi input" id="<?= e($form_id) ?>-phone" type="tel" name="fullphone" placeholder="06 12345678" autocomplete="tel" inputmode="tel">
...
<?php endif; ?>
    ...
    <div class="payrow" aria-label="Geaccepteerde betaalmethoden">
      ...
      <svg class="fai" role="img" aria-label="Bankoverschrijving" viewBox="0 0 512 512"><use href="#i-building-columns"/></svg>
      <svg class="fai pay-ok" role="img" aria-label="Veilig" viewBox="0 0 512 512"><use href="#i-shield-halved"/></svg>
    </div>
    ...
    <p class="jvrey">
      Door verder te gaan, accepteert u het privacybeleid en de gebruiksvoorwaarden.
