...
$form_submit = $form_submit ?? 'Şimdi başlayın';
...
>
  <div class="form-already-registered hidden" data-already-registered>
    ...
    <p class="form-already-registered__text">
      <?= e(SITE_NAME) ?> alındı. Lütfen danışmanımızın aramasını bekleyin.
    </p>
  </div>
...
<?php if ($is_stack): ?>
    <div class="xdzqh">
      <div class="wxts7 field" data-field="fname">
        <label class="field-label" for="<?= e($form_id) ?>-fname">Ad</label>
        <input class="wnzxg input" id="<?= e($form_id) ?>-fname" type="text" name="fname" placeholder="Ad" required pattern="[^\d]+" autocomplete="given-name">
      </div>
      <div class="wxts7 field" data-field="lname">
        <label class="field-label" for="<?= e($form_id) ?>-lname">Soyad</label>
        <input class="wnzxg input" id="<?= e($form_id) ?>-lname" type="text" name="lname" placeholder="Soyad" required pattern="[^\d]+" autocomplete="family-name">
      </div>
    </div>
    <div class="wxts7 field" data-field="email">
      <label class="field-label" for="<?= e($form_id) ?>-email">E-posta</label>
      <input class="wnzxg input" id="<?= e($form_id) ?>-email" type="email" name="email" placeholder="E-posta" required autocomplete="email" inputmode="email">
    </div>
...
<?php endif; ?>
    ...
    <div class="nmpf6" aria-label="Kabul edilen ödeme yöntemleri">
      ...
      <svg class="ajr3je" role="img" aria-label="Banka havalesi" viewBox="0 0 512 512"><use href="#i-building-columns"/></svg>
      <svg class="ajr3je kw4ijl" role="img" aria-label="Güvenli" viewBox="0 0 512 512"><use href="#i-shield-halved"/></svg>
    </div>
