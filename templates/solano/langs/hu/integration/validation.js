...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Adja meg a telefonszámát';
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'A munkamenet lejárt. Frissítse az oldalt, és próbálja újra.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Valami hiba történt. Próbálja újra később.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Kapcsolódási hiba. Ellenőrizze az internetet, és próbálja újra.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
