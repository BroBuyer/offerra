...
$form_submit = $form_submit ?? 'Začať teraz';
...
>
  <div class="form-already-registered hidden" data-already-registered>
    ...
    <p class="form-already-registered__text">
      Vašu žiadosť u <?= e(SITE_NAME) ?> sme prijali. Počkajte, prosím, na hovor od nášho poradcu.
    </p>
  </div>
...
<?php if ($is_stack): ?>
    <div class="xdzqh">
      <div class="wxts7 field" data-field="fname">
        <label class="field-label" for="<?= e($form_id) ?>-fname">Meno</label>
        <input class="wnzxg input" id="<?= e($form_id) ?>-fname" type="text" name="fname" placeholder="Meno" required pattern="[^\d]+" autocomplete="given-name">
      </div>
      <div class="wxts7 field" data-field="lname">
        <label class="field-label" for="<?= e($form_id) ?>-lname">Priezvisko</label>
        <input class="wnzxg input" id="<?= e($form_id) ?>-lname" type="text" name="lname" placeholder="Priezvisko" required pattern="[^\d]+" autocomplete="family-name">
      </div>
    </div>
    <div class="wxts7 field" data-field="email">
      <label class="field-label" for="<?= e($form_id) ?>-email">E-mail</label>
      <input class="wnzxg input" id="<?= e($form_id) ?>-email" type="email" name="email" placeholder="E-mail" required autocomplete="email" inputmode="email">
    </div>
    <div class="wxts7 field" data-field="phone">
      <label class="field-label" for="<?= e($form_id) ?>-phone">Telefón</label>
...
<?php endif; ?>
    ...
    <div class="nmpf6" aria-label="Prijímané platobné metódy">
      ...
      <svg class="ajr3je" role="img" aria-label="Bankový prevod" viewBox="0 0 512 512"><use href="#i-building-columns"/></svg>
      <svg class="ajr3je kw4ijl" role="img" aria-label="Zabezpečené" viewBox="0 0 512 512"><use href="#i-shield-halved"/></svg>
    </div>
    ...
    <p class="ac7ftax">
      Pokračovaním prijímate zásady ochrany súkromia a podmienky používania.
