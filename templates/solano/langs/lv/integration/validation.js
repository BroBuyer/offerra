...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Ievadiet tālruņa numuru';
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Sesija beigusies. Lūdzu, pārlādējiet lapu un mēģiniet vēlreiz.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Kaut kas nogāja greizi. Lūdzu, mēģiniet vēlāk.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Savienojuma kļūda. Pārbaudiet interneta savienojumu un mēģiniet vēlreiz.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
