...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Introduceți numărul de telefon';
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Sesiunea a expirat. Reîncărcați pagina și încercați din nou.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Ceva nu a mers bine. Încercați din nou mai târziu.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Eroare de conexiune. Verificați internetul și încercați din nou.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
