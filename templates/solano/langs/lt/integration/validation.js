...
const phoneErrorMap = {
  0: L.valTelefonasInvalid || 'Įveskite galiojantį telefono numerį',
  1: L.valTelefonasCountry || 'Netinkamas šalies kodas',
  2: L.valTelefonasShort || 'Telefono numeris per trumpas',
  3: L.valTelefonasLong || 'Telefono numeris per ilgas',
  4: L.valTelefonasInvalid || 'Įveskite galiojantį telefono numerį',
};
...
function validateTelefonas(phoneInput, iti) {
  const trimmed = phoneInput.value.trim();
  if (!trimmed) return L.valTelefonasRequired || 'Įveskite savo telefono numerį';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valTelefonasInvalid || 'Įveskite galiojantį telefono numerį';
  }
  return '';
}
...
async function resolveVisitorTelefonasCountry(form, onlyCountries) {
  try {
...
function syncTelefonasCountryHidden(form, iti) {
...
function setupFormValidation(form) {
  ...
  const iti = window.intlTelInput(phone, {
    ...
    customPlaceholder: () => '612 34 56 78',
  });

  if (!phone.getAttribute('placeholder')) {
    phone.setAttribute('placeholder', '612 34 56 78');
  }
  ...
  if (singleCountry) {
    ...
    resolveVisitorTelefonasCountry(form, onlyCountries).then((code) => {
      if (code && code !== phoneCountry) {
        iti.setCountry(code);
        syncTelefonasCountryHidden(form, iti);
      }
    });
  }

  phone.addEventListener('countrychange', () => syncTelefonasCountryHidden(form, iti));
  ...
  phone.addEventListener('blur', () => {
    ...
    const msg = validateTelefonas(phone, iti);
  ...
  form.addEventListener('submit', async (e) => {
    ...
    clearFieldError(phone);
    const phoneError = validateTelefonas(phone, iti);
    if (phoneError) {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Sesija baigėsi. Įkelkite puslapį iš naujo ir bandykite dar kartą.',
      );
    ...
    syncTelefonasCountryHidden(form, iti);
    const fullTelefonas = iti.getNumber();
    const formData = new FormData(form);
    formData.set('fullphone', fullTelefonas);
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Kažkas nutiko. Bandykite dar kartą vėliau.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Ryšio klaida. Patikrinkite interneto ryšį ir bandykite dar kartą.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
