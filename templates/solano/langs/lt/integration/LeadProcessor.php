...
final class LeadProcessor
{
    ...
    public static function normalizeTelefonas(string $phoneRaw): string
    {
        $phone = trim($phoneRaw);
        if ($phone === '') {
            return '';
    ...
    public static function ipCountryMatchesPasiūlyti(): bool
    {
        ...
        $ipTelefonasCode = self::ipToTelefonasCountryCode($ipCountry);

        return $ipTelefonasCode !== '' && in_array($ipTelefonasCode, $allowed, true);
    }

    private static function ipToTelefonasCountryCode(string $ipCountry): string
    {
    ...
    public static function buildCrmPayload(array $lead): array
    {
        ...
        $payload = [
            ...
            'phone' => self::normalizeTelefonas((string) ($lead['phone'] ?? $lead['fullphone'] ?? '')),
    ...
    {
        if (self::ipCountryMatchesPasiūlyti()) {
            return null;
        }
    ...
    public static function buildTelegramMessage(
        ...
        $lines = [
            ...
            '<b>El. paštas:</b> ' . self::escapeTelegramHtml((string) ($crmData['email'] ?? '')),
            '<b>Telefonas:</b> ' . self::escapeTelegramHtml((string) ($crmData['phone'] ?? '')),
            '',
            ...
            '<b>Pasiūlyti:</b> ' . self::escapeTelegramHtml(SITE_NAME),
