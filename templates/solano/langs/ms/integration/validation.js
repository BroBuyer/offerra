...
const phoneErrorMap = {
  0: L.valTelefonInvalid || 'Masukkan nombor telefon yang sah',
  1: L.valTelefonCountry || 'Kod negara tidak sah',
  2: L.valTelefonShort || 'Nombor telefon terlalu pendek',
  3: L.valTelefonLong || 'Nombor telefon terlalu panjang',
  4: L.valTelefonInvalid || 'Masukkan nombor telefon yang sah',
};
...
function validateTelefon(phoneInput, iti) {
  ...
  if (!trimmed) return L.valTelefonRequired || 'Masukkan nombor telefon anda';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valTelefonInvalid || 'Masukkan nombor telefon yang sah';
  }
  return '';
}
...
async function resolveVisitorTelefonCountry(form, onlyCountries) {
  try {
...
function syncTelefonCountryHidden(form, iti) {
...
function setupFormValidation(form) {
  ...
  if (singleCountry) {
    ...
    resolveVisitorTelefonCountry(form, onlyCountries).then((code) => {
      if (code && code !== phoneCountry) {
        iti.setCountry(code);
        syncTelefonCountryHidden(form, iti);
      }
    });
  }

  phone.addEventListener('countrychange', () => syncTelefonCountryHidden(form, iti));
  ...
  phone.addEventListener('blur', () => {
    ...
    const msg = validateTelefon(phone, iti);
  ...
  form.addEventListener('submit', async (e) => {
    ...
    clearFieldError(phone);
    const phoneError = validateTelefon(phone, iti);
    if (phoneError) {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Sesi tamat. Muat semula halaman dan cuba lagi.',
      );
    ...
    syncTelefonCountryHidden(form, iti);
    const fullTelefon = iti.getNumber();
    const formData = new FormData(form);
    formData.set('fullphone', fullTelefon);
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Sesuatu tidak kena. Sila cuba lagi kemudian.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Ralat sambungan. Semak sambungan internet anda dan cuba lagi.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
