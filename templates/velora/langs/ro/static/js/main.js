(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Întunecat';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Comută la tema întunecată');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Luminos';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Comută la tema luminoasă');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'ordin în așteptarea alocării');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Astăzi');
