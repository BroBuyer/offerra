...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Adjon meg érvényes telefonszámot',
  1: L.valPhoneCountry || 'Érvénytelen országkód',
  2: L.valPhoneShort || 'A telefonszám túl rövid',
  3: L.valPhoneLong || 'A telefonszám túl hosszú',
  4: L.valPhoneInvalid || 'Adjon meg érvényes telefonszámot',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Adja meg telefonszámát';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Adjon meg érvényes telefonszámot';
  }
  return '';
}
...
/** Prefer token platformon page-load prefetch; only refetch if missing. Honour min_age. */
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'A munkamenet lejárt. Kérjük, töltse újra az oldalt, és próbálja újra.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Valami hiba történt. Kérjük, próbálja újra később.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Kapcsolati hiba. Ellenőrizze internetkapcsolatát, és próbálja újra.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
