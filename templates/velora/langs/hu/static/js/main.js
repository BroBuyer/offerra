(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Sötét';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Váltás sötét témára');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Világos';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Váltás világos témára');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'megbízás kiosztásra vár');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Ma');
