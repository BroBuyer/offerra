...
function validateTelepon(phoneInput, iti) {
  ...
  if (!trimmed) return L.valTeleponRequired || 'Masukkan nomor telepon Anda';
...
/** Prefer token dari page-load prefetch; only refetch if missing. Honour min_age. */
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Sesi berakhir. Muat ulang halaman dan coba lagi.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Terjadi kesalahan. Silakan coba lagi nanti.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Kesalahan koneksi. Periksa koneksi internet anda dan coba lagi.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
