...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Zadajte platné telefónne číslo',
  1: L.valPhoneCountry || 'Neplatný kód krajiny',
  2: L.valPhoneShort || 'Telefónne číslo je príliš krátke',
  3: L.valPhoneLong || 'Telefónne číslo je príliš dlhé',
  4: L.valPhoneInvalid || 'Zadajte platné telefónne číslo',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Zadajte svoje telefónne číslo';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Zadajte platné telefónne číslo';
  }
  return '';
}
...
/** Prefer token od page-load prefetch; only refetch if missing. Honour min_age. */
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Relácia vypršala. Načítajte prosím stránku znova a skúste to znova.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Niečo sa pokazilo. Skúste to prosím neskôr.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Chyba pripojenia. Skontrolujte internetové pripojenie a skúste to znova.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
