(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      ...
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Prepnúť na tmavý režim');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Svetlý';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Prepnúť na svetlý režim');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'objednávka čaká na alokáciu');
      }
