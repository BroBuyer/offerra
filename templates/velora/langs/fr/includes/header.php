...
<header class="header site-header" data-header>
  <div class="container" style="display:flex;align-items:center;justify-content:space-between;gap:20px;min-height:94px;">
    ...
    <nav class="nav nav-desktop" id="mainNav" aria-label="Navigation principale">
      <a href="<?= e($sec('#security')) ?>" class="nav-link">Sécurité</a>
      <a href="<?= e($sec('#reviews')) ?>" class="nav-link">Avis</a>
      <a href="<?= e($sec('#faq')) ?>" class="nav-link">FAQ</a>
      <a href="product.php" class="nav-link<?= ($active_page ?? '') === 'product' ? ' is-active' : '' ?>">À propos</a>
      <a href="contacts.php" class="nav-link<?= ($active_page ?? '') === 'contacts' ? ' is-active' : '' ?>">Contact</a>
    </nav>

    <div style="display:flex;gap:14px;align-items:center;">
      <button class="theme-toggle" id="themeToggle" type="button" aria-label="Passer au thème clair">☀️ Clair</button>
      <a href="<?= e($sec('#signup')) ?>" class="btn btn-primary header-cta-btn">Démarrer le trading</a>
      <button class="burger menu-toggle" id="burgerBtn" type="button" data-menu-toggle aria-label="Ouvrir le menu" aria-expanded="false">
  ...
  <nav class="nav-mobile" data-mobile-nav aria-label="Navigation mobile" hidden>
    <a href="<?= e($sec('#security')) ?>">Sécurité</a>
    <a href="<?= e($sec('#reviews')) ?>">Avis</a>
    <a href="<?= e($sec('#faq')) ?>">FAQ</a>
    <a href="product.php">À propos</a>
    <a href="contacts.php">Contact</a>
    <a href="<?= e($sec('#signup')) ?>" class="btn btn-primary">Démarrer le trading</a>
  </nav>
</header>
