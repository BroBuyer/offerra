...
<header class="header site-header" data-header>
  <div class="container" style="display:flex;align-items:center;justify-content:space-between;gap:20px;min-height:94px;">
    <a href="<?= page_url() ?>" class="logo" aria-label="<?= e(SITE_NAME) ?> startpagina">
    ...
    <nav class="nav nav-desktop" id="mainNav" aria-label="Hoofdnavigatie">
      <a href="<?= e($sec('#security')) ?>" class="nav-link">Beveiliging</a>
      <a href="<?= e($sec('#reviews')) ?>" class="nav-link">Reviews</a>
      <a href="<?= e($sec('#faq')) ?>" class="nav-link">FAQ</a>
      <a href="product.php" class="nav-link<?= ($active_page ?? '') === 'product' ? ' is-active' : '' ?>">Over ons</a>
      <a href="contacts.php" class="nav-link<?= ($active_page ?? '') === 'contacts' ? ' is-active' : '' ?>">Contact</a>
    </nav>

    <div style="display:flex;gap:14px;align-items:center;">
      <button class="theme-toggle" id="themeToggle" type="button" aria-label="Overschakelen naar licht thema">☀️ Licht</button>
      <a href="<?= e($sec('#signup')) ?>" class="btn btn-primary header-cta-btn">Begin met handelen</a>
      <button class="burger menu-toggle" id="burgerBtn" type="button" data-menu-toggle aria-label="Menu openen" aria-expanded="false">
        <span></span><span></span><span></span>
      </button>
    </div>
  </div>

  <nav class="nav-mobile" data-mobile-nav aria-label="Mobiele navigatie" hidden>
    <a href="<?= e($sec('#security')) ?>">Beveiliging</a>
    <a href="<?= e($sec('#reviews')) ?>">Reviews</a>
    <a href="<?= e($sec('#faq')) ?>">FAQ</a>
    <a href="product.php">Over ons</a>
    <a href="contacts.php">Contact</a>
    <a href="<?= e($sec('#signup')) ?>" class="btn btn-primary">Begin met handelen</a>
  </nav>
</header>
