(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Donker';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Overschakelen naar donker thema');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Licht';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Overschakelen naar licht thema');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'order in afwachting van toewijzing');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Vandaag');
