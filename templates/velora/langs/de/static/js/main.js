(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Dunkel';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Zum dunklen Design wechseln');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Hell';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Zum hellen Design wechseln');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'Auftrag wartet auf Zuteilung');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Heute');
