...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Geben Sie eine gültige Telefonnummer ein',
  ...
  4: L.valPhoneInvalid || 'Geben Sie eine gültige Telefonnummer ein',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Geben Sie Ihre Telefonnummer ein';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Geben Sie eine gültige Telefonnummer ein';
  }
  return '';
}
...
/** Prefer token ab page-load prefetch; only refetch if missing. Honour min_age. */
