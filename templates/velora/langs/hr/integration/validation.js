...
const phoneErrorMap = {
  0: 'Unesite valjani broj telefona',
  1: 'Nevažeći pozivni broj države',
  2: 'Broj telefona je prekratak',
  3: 'Broj telefona je predugačak',
  4: 'Unesite valjani broj telefona',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Unesite svoj broj telefona';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Unesite valjani broj telefona';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, 'Sesija je istekla. Ponovno učitajte stranicu i pokušajte ponovo.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Nešto je pošlo po zlu. Pokušajte ponovo kasnije.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Greška veze. Provjerite internetsku vezu i pokušajte ponovo.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
