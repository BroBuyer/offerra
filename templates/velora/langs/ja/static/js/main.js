(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 ダーク';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'ダークテーマに切り替える');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ ライト';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'ライトテーマに切り替える');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || '注文は割り当て待ちです');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || '本日');
