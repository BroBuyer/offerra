  ...
  const steps = [
    {
      bot:
        L.chatStep1Bot ||
        "Sveiki! Aš esu Lisa, jūsų įstojimo padėjėja. Pasiruošę atidaryti prekybos sąskaitą keliais greitais žingsniais?",
      choices: [
        { label: L.chatStep1Yes || "Taip, pradėkime", value: 'start' },
        { label: L.chatStep1More || 'Pirmiausia papasakok daugiau', value: 'more' },
      ],
    },
    {
      bot: L.chatStep2Bot || 'Puiku. Ar anksčiau prekiavote kriptovaliuta ar forex?',
      choices: [
        { label: L.chatStep2New || "esu naujokas", value: 'beginner' },
        { label: L.chatStep2Mid || 'Tam tikra patirtis', value: 'mid' },
        { label: L.chatStep2Pro || "Aš patyręs", value: 'pro' },
      ],
    },
    {
      bot: L.chatStep3Bot || 'Kas jus šiuo metu domina labiausiai?',
      choices: [
        { label: L.chatStep3Crypto || 'Kripto', value: 'crypto' },
        { label: L.chatStep3Forex || 'Forex', value: 'forex' },
        { label: L.chatStep3Stocks || 'Akcijos / indeksai', value: 'stocks' },
        { label: L.chatStep3All || 'Visa tai, kas išdėstyta aukščiau', value: 'all' },
      ],
    },
    {
      bot:
        L.chatStep4Bot ||
        "Tobulas. Parengsiu nemokamą paskyros formą – tai užtruks mažiau nei 3 minutes ir mūsų komanda paskambins, kad užbaigtų sąranką.",
      choices: [{ label: L.chatStep4Form || 'Atidarykite formą', value: 'form' }],
    },
  ];

  const moreReply =
    L.chatMoreReply ||
    'Pradedantiesiems pateikiame švarų prietaisų skydelį, AI rinkos patarimus paprasta kalba ir saugų finansavimą iš jūsų minimalaus įnašo. Ar tęsime?';

  const continueLabel = L.chatContinue || "Taip, tęskime";
  const formPrompt =
    L.chatFormPrompt ||
    "Įveskite savo duomenis žemiau ir pateikite – jei ko prireiks, pasiliksiu čia.";
