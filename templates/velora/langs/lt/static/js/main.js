(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Tamsu';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Perjungti į tamsią temą');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Šviesa';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Perjungti į šviesią temą');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'užsakymas laukia paskirstymo');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Šiandien');
