(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Gelap';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Tukar ke tema gelap');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Cerah';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Tukar ke tema cerah');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'pesanan menunggu peruntukan');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Hari ini');
