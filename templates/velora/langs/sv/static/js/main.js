(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Mörkt';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Byt till mörkt tema');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Ljust';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Byt till ljust tema');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'order väntar på allokering');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Idag');
