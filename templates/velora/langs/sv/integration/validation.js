...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Ange ditt telefonnummer';
...
/** Prefer token från page-load prefetch; only refetch if missing. Honour min_age. */
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Sessionen har gått ut. Ladda om sidan och försök igen.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Något gick fel. Försök igen senare.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Anslutningsfel. Kontrollera din internetanslutning och försök igen.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
