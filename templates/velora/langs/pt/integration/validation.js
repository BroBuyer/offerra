...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Introduza um número de telefone válido',
  ...
  4: L.valPhoneInvalid || 'Introduza um número de telefone válido',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Introduza o seu número de telefone';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Introduza um número de telefone válido';
  }
  return '';
}
...
/** Prefer token a partir de page-load prefetch; only refetch if missing. Honour min_age. */
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Sessão expirada. Recarregue a página e tente novamente.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Algo correu mal. Tente novamente mais tarde.',
        );
        return;
      }
