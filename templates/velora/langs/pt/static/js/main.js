(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Escuro';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Mudar para o tema escuro');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Claro';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Mudar para o tema claro');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'ordem pendente de alocação');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Hoje');
