...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Syötä kelvollinen puhelinnumero',
  1: L.valPhoneCountry || 'Virheellinen maakoodi',
  2: L.valPhoneShort || 'Puhelinnumero on liian lyhyt',
  3: L.valPhoneLong || 'Puhelinnumero on liian pitkä',
  4: L.valPhoneInvalid || 'Syötä kelvollinen puhelinnumero',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Syötä puhelinnumerosi';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Syötä kelvollinen puhelinnumero';
  }
  return '';
}
...
/** Prefer token alkaen page-load prefetch; only refetch if missing. Honour min_age. */
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Istunto vanhentui. Lataa sivu uudelleen ja yritä uudelleen.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Jokin meni pieleen. Yritä myöhemmin uudelleen.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Yhteysvirhe. Tarkista internetyhteytesi ja yritä uudelleen.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
