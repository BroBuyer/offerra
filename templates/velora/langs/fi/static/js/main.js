(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Tumma';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Vaihda tummaan teemaan');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Vaalea';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Vaihda vaaleaan teemaan');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'toimeksianto odottaa kohdistusta');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Tänään');
