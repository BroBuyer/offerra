(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Σκοτεινό';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Μετάβαση στο σκοτεινό θέμα');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Φωτεινό';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Μετάβαση στο φωτεινό θέμα');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'εκκρεμής εντολή κατανομής');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Σήμερα');
