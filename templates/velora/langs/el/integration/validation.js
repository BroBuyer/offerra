...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Εισαγάγετε τον αριθμό τηλεφώνου σας';
...
/** Prefer token από page-load prefetch; only refetch if missing. Honour min_age. */
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Η συνεδρία έληξε. Φορτώστε ξανά τη σελίδα και δοκιμάστε ξανά.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Κάτι πήγε στραβά. Δοκιμάστε ξανά αργότερα.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Σφάλμα σύνδεσης. Ελέγξτε τη σύνδεσή σας στο internet και δοκιμάστε ξανά.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
