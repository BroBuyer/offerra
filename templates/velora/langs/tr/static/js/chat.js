  ...
  const steps = [
    {
      bot:
        L.chatStep1Bot ||
        "Merhaba! Ben Lisa, katılım asistanınızım. Birkaç hızlı adımda işlem hesabı açmaya hazır mısınız?",
      choices: [
        { label: L.chatStep1Yes || "Evet, başlayalım", value: 'start' },
        { label: L.chatStep1More || 'Önce daha fazla bilgi ver', value: 'more' },
      ],
    },
    {
      bot: L.chatStep2Bot || 'Harika. Daha önce kripto veya forex işlemi yaptınız mı?',
      choices: [
        { label: L.chatStep2New || "Yeniyim", value: 'beginner' },
        { label: L.chatStep2Mid || 'Biraz deneyimim var', value: 'mid' },
        { label: L.chatStep2Pro || "Deneyimliyim", value: 'pro' },
      ],
    },
    {
      bot: L.chatStep3Bot || 'Şu anda en çok ne ilginizi çekiyor?',
      choices: [
        { label: L.chatStep3Crypto || 'Kripto', value: 'crypto' },
        { label: L.chatStep3Forex || 'Forex', value: 'forex' },
        { label: L.chatStep3Stocks || 'Hisse / endeksler', value: 'stocks' },
        { label: L.chatStep3All || 'Hepsi', value: 'all' },
      ],
    },
    {
      bot:
        L.chatStep4Bot ||
        "Mükemmel. Ücretsiz bir hesap formu hazırlayacağım — 3 dakikadan kısa sürer ve ekibimiz kurulumu tamamlamak için arayacak.",
      choices: [{ label: L.chatStep4Form || 'Formu aç', value: 'form' }],
    },
  ];

  const moreReply =
    L.chatMoreReply ||
    'Yeni başlayanlara sade bir pano, sade dilde AI piyasa ipuçları ve minimum yatırımınızdan güvenli fonlama ile rehberlik ediyoruz. Devam edelim mi?';

  const continueLabel = L.chatContinue || "Evet, devam edelim";
  const formPrompt =
    L.chatFormPrompt ||
    "Lütfen bilgilerinizi aşağıya girin ve gönderin — bir şeye ihtiyacınız olursa burada kalacağım.";
