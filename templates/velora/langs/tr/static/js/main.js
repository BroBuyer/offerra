(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Koyu';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Koyu temaya geç');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Açık';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Açık temaya geç');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'emir tahsis bekliyor');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Bugün');
