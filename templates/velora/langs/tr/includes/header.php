...
<header class="header site-header" data-header>
  <div class="container" style="display:flex;align-items:center;justify-content:space-between;gap:20px;min-height:94px;">
    ...
    <nav class="nav nav-desktop" id="mainNav" aria-label="Ana navigasyon">
      <a href="<?= e($sec('#security')) ?>" class="nav-link">Güvenlik</a>
      <a href="<?= e($sec('#reviews')) ?>" class="nav-link">Yorumlar</a>
      <a href="<?= e($sec('#faq')) ?>" class="nav-link">FAQ</a>
      <a href="product.php" class="nav-link<?= ($active_page ?? '') === 'product' ? ' is-active' : '' ?>">Hakkımızda</a>
      <a href="contacts.php" class="nav-link<?= ($active_page ?? '') === 'contacts' ? ' is-active' : '' ?>">İletişim</a>
    </nav>

    <div style="display:flex;gap:14px;align-items:center;">
      <button class="theme-toggle" id="themeToggle" type="button" aria-label="Açık temaya geç">☀️ Açık</button>
      <a href="<?= e($sec('#signup')) ?>" class="btn btn-primary header-cta-btn">İşlem yapmaya başla</a>
      <button class="burger menu-toggle" id="burgerBtn" type="button" data-menu-toggle aria-label="Menüyü aç" aria-expanded="false">
  ...
  <nav class="nav-mobile" data-mobile-nav aria-label="Mobil navigasyon" hidden>
    <a href="<?= e($sec('#security')) ?>">Güvenlik</a>
    <a href="<?= e($sec('#reviews')) ?>">Yorumlar</a>
    <a href="<?= e($sec('#faq')) ?>">FAQ</a>
    <a href="product.php">Hakkımızda</a>
    <a href="contacts.php">İletişim</a>
    <a href="<?= e($sec('#signup')) ?>" class="btn btn-primary">İşlem yapmaya başla</a>
  </nav>
</header>
