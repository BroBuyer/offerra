...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Geçerli bir telefon numarası girin',
  ...
  4: L.valPhoneInvalid || 'Geçerli bir telefon numarası girin',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Telefon numaranızı girin';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Geçerli bir telefon numarası girin';
  }
  return '';
}
...
/** Prefer token 'den itibaren page-load prefetch; only refetch if missing. Honour min_age. */
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Oturum süresi doldu. Lütfen sayfayı yenileyin ve tekrar deneyin.',
      );
