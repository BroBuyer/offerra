...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Zadejte platné telefonní číslo',
  1: L.valPhoneCountry || 'Neplatný kód země',
  2: L.valPhoneShort || 'Telefonní číslo je příliš krátké',
  3: L.valPhoneLong || 'Telefonní číslo je příliš dlouhé',
  4: L.valPhoneInvalid || 'Zadejte platné telefonní číslo',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneRequired || 'Zadejte své telefonní číslo';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Zadejte platné telefonní číslo';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(
        form,
        L.valSessionExpired || 'Relace vypršela. Načtěte prosím stránku znovu a zkuste to znovu.',
      );
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(
          form,
          data.error || L.valGenericError || 'Něco se pokazilo. Zkuste to prosím později.',
        );
        return;
      }
      ...
      showFormMessage(
        ...
        L.valConnectionError ||
          'Chyba připojení. Zkontrolujte internetové připojení a zkuste to znovu.',
      );
    } finally {
      preloader?.classList.add('hidden');
    }
