...
<header class="header site-header" data-header>
  <div class="container" style="display:flex;align-items:center;justify-content:space-between;gap:20px;min-height:94px;">
    ...
    <nav class="nav nav-desktop" id="mainNav" aria-label="Hlavní navigace">
      <a href="<?= e($sec('#security')) ?>" class="nav-link">Zabezpečení</a>
      <a href="<?= e($sec('#reviews')) ?>" class="nav-link">Recenze</a>
      <a href="<?= e($sec('#faq')) ?>" class="nav-link">FAQ</a>
      <a href="product.php" class="nav-link<?= ($active_page ?? '') === 'product' ? ' is-active' : '' ?>">O nás</a>
      <a href="contacts.php" class="nav-link<?= ($active_page ?? '') === 'contacts' ? ' is-active' : '' ?>">Kontakt</a>
    </nav>

    <div style="display:flex;gap:14px;align-items:center;">
      <button class="theme-toggle" id="themeToggle" type="button" aria-label="Přepnout na světlý režim">☀️ Světlý</button>
      <a href="<?= e($sec('#signup')) ?>" class="btn btn-primary header-cta-btn">Začít obchodovat</a>
      <button class="burger menu-toggle" id="burgerBtn" type="button" data-menu-toggle aria-label="Otevřít menu" aria-expanded="false">
        <span></span><span></span><span></span>
      </button>
    </div>
  </div>

  <nav class="nav-mobile" data-mobile-nav aria-label="Mobilní navigace" hidden>
    <a href="<?= e($sec('#security')) ?>">Zabezpečení</a>
    <a href="<?= e($sec('#reviews')) ?>">Recenze</a>
    <a href="<?= e($sec('#faq')) ?>">FAQ</a>
    <a href="product.php">O nás</a>
    <a href="contacts.php">Kontakt</a>
    <a href="<?= e($sec('#signup')) ?>" class="btn btn-primary">Začít obchodovat</a>
  </nav>
</header>
