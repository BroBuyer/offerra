(() => {
  ...
  const applyTheme = (theme) => {
    ...
    if (theme === 'light') {
      themeToggle.textContent = lang.themeToggleDarkText || '🌙 Tmavý';
      themeToggle.setAttribute('aria-label', lang.themeToggleDarkAria || 'Přepnout na tmavý režim');
    } else {
      themeToggle.textContent = lang.themeToggleLightText || '☀️ Světlý';
      themeToggle.setAttribute('aria-label', lang.themeToggleLightAria || 'Přepnout na světlý režim');
    }
  };
  ...
  document.querySelectorAll('[data-mock-action], .m-btn-sell, .m-btn-buy').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (headline) {
        ...
        headline.textContent = label + ' ' + (lang.orderPendingAllocation || 'objednávka čeká na alokaci');
      }
    ...
    if (chartTrack && chartBars.length) {
      ...
      if (mockupChange) {
        ...
        mockupChange.textContent =
          (pct >= 0 ? '+' : '') + pct.toFixed(2) + '% ' + (lang.mockupToday || 'Dnes');
