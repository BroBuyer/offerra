<?php
require_once __DIR__ . '/includes/config.php';
$page_title = page_title('Adatvédelmi irányelvek');
$page_description = 'Hogyan gyűjti, használja és védi a ' . SITE_NAME . ' az Ön személyes adatait.';
$page_canonical = page_url('privacy.php');
$active_page = 'privacy';
require __DIR__ . '/includes/head.php';
?>
<header class="site-head">
  <div class="shell nav">
    <a class="brand" href="<?= page_url() ?>">
      <img src="<?= asset('static/img/brand/logo.webp') ?>" alt="<?= e(SITE_NAME) ?>" width="30" height="30" decoding="async" loading="eager">
      <?= e(SITE_NAME) ?>
    </a>
    <nav class="nav-links" aria-label="Főoldal">
      <a href="<?= page_url() ?>#platform">Platform</a>
      <a href="<?= page_url() ?>#how">Hogyan működik</a>
      <a href="<?= page_url() ?>#markets">Piacok</a>
      <a href="<?= page_url() ?>#faq">Gyakran ismételt kérdések</a>
    </nav>
    <div class="nav-cta">
      <a class="btn btn-primary" href="<?= page_url() ?>#signup">Kezdés</a>
    </div>
  </div>
</header>

<main id="main">
  <section class="legal-hero">
    <div class="shell">
      <span class="eyebrow">Jogi információk</span>
      <h1>Adatvédelmi irányelvek</h1>
      <p class="lede">Hogyan gyűjti, használja és védi a <?= e(SITE_NAME) ?> az Ön személyes adatait.</p>
    </div>
  </section>

  <section class="legal-body">
    <div class="shell">
      <p class="meta">Utolsó frissítés: 2025. január</p>

      <h2>1. Bevezetés</h2>
      <p>A <?= e(SITE_NAME) ?> („mi”, „miénk”) elkötelezett a látogatók és ügyfelek adatainak védelme mellett. Ez az Adatvédelmi irányelv elmagyarázza, milyen személyes adatokat gyűjtünk, miért és hogyan kezeljük őket, amikor weboldalunkat és szolgáltatásainkat használja.</p>

      <h2>2. Az általunk gyűjtött információk</h2>
      <ul>
        <li>Személyazonosító adatok — név, születési dátum, államilag kiállított igazolványok a regisztrációhoz és KYC/AML megfeleléshez.</li>
        <li>Kapcsolattartási adatok — e-mail-cím, telefonszám, postacím.</li>
        <li>Pénzügyi adatok — fizetési részletek, tranzakciós előzmények, a források származására vonatkozó információk.</li>
        <li>Technikai adatok — IP-cím, böngészőtípus, eszközazonosítók, sütik és használati elemzések.</li>
      </ul>

      <h2>3. Hogyan használjuk adatait</h2>
      <ul>
        <li>A személyazonosság ellenőrzésére és a szabályozási kötelezettségek teljesítésére.</li>
        <li>Platformunk és szolgáltatásaink nyújtására, fenntartására és fejlesztésére.</li>
        <li>Fizetések feldolgozására és csalárd tevékenység felismerésére.</li>
        <li>Az Önnel való kommunikációra a fiókjáról, frissítésekről és támogatási kérésekről.</li>
        <li>Marketingkommunikáció küldésére, ha hozzájárult (bármikor leiratkozhat).</li>
      </ul>

      <h2>4. Jogi alap</h2>
      <p>A személyes adatokat a következő jogszerű alapok egyike vagy többike alapján kezeljük: szerződés teljesítése, jogi kötelezettség teljesítése, jogos érdekeink vagy az Ön hozzájárulása.</p>

      <h2>5. Megosztás és közzététel</h2>
      <p>Személyes adatokat megoszthatunk szabályozott fizetési szolgáltatókkal, KYC/AML ellenőrző partnerekkel, felhőinfrastruktúra-szolgáltatókkal, szakmai tanácsadókkal és illetékes hatóságokkal, ha a törvény megköveteli. Személyes adatokat nem értékesítünk.</p>

      <h2>6. Nemzetközi továbbítások</h2>
      <p>Amikor személyes adatokat az Ön joghatóságán kívülre továbbítunk, megfelelő garanciákat biztosítunk, beleértve a standard szerződéses záradékokat.</p>

      <h2>7. Adatmegőrzés</h2>
      <p>A személyes adatokat addig őrizzük, amíg a szolgáltatások nyújtásához, valamint a jogi, szabályozási és számviteli követelmények teljesítéséhez szükséges — jellemzően legalább öt évig a fiók lezárása után.</p>

      <h2>8. Az Ön jogai</h2>
      <p>A vonatkozó jog függvényében kérheti személyes adataihoz való hozzáférést, helyesbítést, törlést, korlátozást vagy hordozhatóságot, és bizonyos kezelés ellen tiltakozhat. E jogok gyakorlásához vegye fel velünk a kapcsolatot az alábbi címen.</p>

      <h2>9. Sütik</h2>
      <p>Alapvető és analitikai sütiket használunk az oldal működtetéséhez és a használat megértéséhez. A sütiket a böngészőbeállításokban kezelheti.</p>

      <h2>10. Biztonság</h2>
      <p>Adminisztratív, technikai és fizikai védelmet alkalmazunk a személyes adatok jogosulatlan hozzáférés, közzététel, módosítás vagy megsemmisítés ellen. Egyetlen rendszer sem tökéletesen biztonságos, és nem garantálhatunk abszolút biztonságot.</p>

      <h2>11. Változások</h2>
      <p>Ezt az irányelvet időről időre frissíthetjük. A legújabb verzió mindig elérhető lesz ezen az oldalon a frissített dátummal.</p>

      <h2>12. Kapcsolat</h2>
      <p>Adatvédelmi kérdésekben vagy jogai gyakorlásához lépjen kapcsolatba a <?= e(SITE_NAME) ?> adatvédelmi csapatával a <a href="<?= page_url('contacts.php') ?>">kapcsolati oldalon</a>.</p>

      <p style="margin-top:36px"><a class="btn btn-ghost" href="<?= page_url() ?>">← Vissza a kezdőlapra</a></p>
    </div>
  </section>
</main>

<footer class="foot">
  <div class="shell">
    <div class="foot-bottom" style="margin-top:0;border-top:none;padding-top:0">
      © <?= date('Y') ?> <?= e(SITE_NAME) ?>. Minden jog fenntartva ·
      <a href="<?= page_url('privacy.php') ?>">Adatvédelem</a> ·
      <a href="<?= page_url('conditions.php') ?>">Feltételek</a>
    </div>
  </div>
</footer>
<?php require __DIR__ . '/includes/footer.php'; ?>
