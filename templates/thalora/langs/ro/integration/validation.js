...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Introduceți un număr de telefon valid',
  1: L.valPhoneCountry || 'Cod de țară invalid',
  2: L.valPhoneShort || 'Numărul de telefon este prea scurt',
  3: L.valPhoneLong || 'Numărul de telefon este prea lung',
  4: L.valPhoneInvalid || 'Introduceți un număr de telefon valid',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Introduceți numărul de telefon';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Introduceți un număr de telefon valid';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sesiunea a expirat. Reîncărcați pagina și încercați din nou.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Ceva nu a mers bine. Încercați din nou mai târziu.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Eroare de conexiune. Verificați internetul și încercați din nou.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
