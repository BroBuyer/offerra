...
const phoneErrorMap = {
  0: L.valTeleponInvalid || 'Masukkan nomor telepon yang valid',
  1: L.valTeleponCountry || 'Kode negara tidak valid',
  2: L.valTeleponShort || 'Nomor telepon terlalu pendek',
  3: L.valTeleponLong || 'Nomor telepon terlalu panjang',
  4: L.valTeleponInvalid || 'Masukkan nomor telepon yang valid',
};
...
function validateTelepon(phoneInput, iti) {
  const trimmed = phoneInput.value.trim();
  if (!trimmed) return L.valTeleponEmpty || 'Masukkan nomor telepon Anda';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valTeleponInvalid || 'Masukkan nomor telepon yang valid';
  }
  return '';
}
...
async function resolveVisitorTeleponCountry(form, onlyCountries) {
  try {
...
function syncTeleponCountryHidden(form, iti) {
...
function setupFormValidation(form) {
  ...
  if (singleCountry) {
    ...
    resolveVisitorTeleponCountry(form, onlyCountries).then((code) => {
      if (code && code !== phoneCountry) {
        iti.setCountry(code);
        syncTeleponCountryHidden(form, iti);
      }
    });
  }

  phone.addEventListener('countrychange', () => syncTeleponCountryHidden(form, iti));
  ...
  phone.addEventListener('blur', () => {
    ...
    const msg = validateTelepon(phone, iti);
  ...
  form.addEventListener('submit', async (e) => {
    ...
    clearFieldError(phone);
    const phoneError = validateTelepon(phone, iti);
    if (phoneError) {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sesi berakhir. Muat ulang halaman dan coba lagi.');
      preloader?.classList.add('hidden');
      return;
    }

    syncTeleponCountryHidden(form, iti);
    const fullTelepon = iti.getNumber();
    const formData = new FormData(form);
    formData.set('fullphone', fullTelepon);
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Terjadi kesalahan. Silakan coba lagi nanti.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Kesalahan koneksi. Periksa koneksi internet Anda dan coba lagi.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
