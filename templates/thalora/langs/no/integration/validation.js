...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Skriv inn et gyldig telefonnummer',
  1: L.valPhoneCountry || 'Ugyldig landskode',
  2: L.valPhoneShort || 'Telefonnummeret er for kort',
  3: L.valPhoneLong || 'Telefonnummeret er for langt',
  4: L.valPhoneInvalid || 'Skriv inn et gyldig telefonnummer',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Skriv inn telefonnummeret ditt';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Skriv inn et gyldig telefonnummer';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Økten er utløpt. Last inn siden på nytt og prøv igjen.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Noe gikk galt. Prøv igjen senere.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Tilkoblingsfeil. Sjekk internettforbindelsen og prøv igjen.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
