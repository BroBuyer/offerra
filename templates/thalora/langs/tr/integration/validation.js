...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Lütfen geçerli bir telefon numarası girin',
  1: L.valPhoneCountry || 'Geçersiz ülke kodu',
  2: L.valPhoneShort || 'Telefon numarası çok kısa',
  3: L.valPhoneLong || 'Telefon numarası çok uzun',
  4: L.valPhoneInvalid || 'Lütfen geçerli bir telefon numarası girin',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Lütfen telefon numaranızı girin';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Lütfen geçerli bir telefon numarası girin';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Oturum süresi doldu. Lütfen sayfayı yenileyip tekrar deneyin.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Bir şeyler ters gitti. Lütfen daha sonra tekrar deneyin.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Bağlantı hatası. İnternetinizi kontrol edin ve tekrar deneyin.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
