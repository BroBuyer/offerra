...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Bitte geben Sie eine gültige Telefonnummer ein',
  1: L.valPhoneCountry || 'Ungültiger Ländercode',
  2: L.valPhoneShort || 'Die Telefonnummer ist zu kurz',
  3: L.valPhoneLong || 'Die Telefonnummer ist zu lang',
  4: L.valPhoneInvalid || 'Bitte geben Sie eine gültige Telefonnummer ein',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Bitte geben Sie Ihre Telefonnummer ein';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Bitte geben Sie eine gültige Telefonnummer ein';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sitzung abgelaufen. Bitte laden Sie die Seite neu und versuchen Sie es erneut.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Etwas ist schiefgelaufen. Bitte versuchen Sie es später erneut.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Verbindungsfehler. Prüfen Sie Ihre Internetverbindung und versuchen Sie es erneut.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
