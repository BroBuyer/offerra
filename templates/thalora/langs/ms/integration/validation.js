...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Masukkan nombor telefon yang sah',
  1: L.valPhoneCountry || 'Kod negara tidak sah',
  2: L.valPhoneShort || 'Nombor telefon terlalu pendek',
  3: L.valPhoneLong || 'Nombor telefon terlalu panjang',
  4: L.valPhoneInvalid || 'Masukkan nombor telefon yang sah',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Masukkan nombor telefon anda';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Masukkan nombor telefon yang sah';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sesi tamat. Muat semula halaman dan cuba lagi.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Sesuatu tidak kena. Sila cuba lagi kemudian.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Ralat sambungan. Semak sambungan internet anda dan cuba lagi.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
