...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Indtast et gyldigt telefonnummer',
  1: L.valPhoneCountry || 'Ugyldig landekode',
  2: L.valPhoneShort || 'Telefonnummeret er for kort',
  3: L.valPhoneLong || 'Telefonnummeret er for langt',
  4: L.valPhoneInvalid || 'Indtast et gyldigt telefonnummer',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Indtast dit telefonnummer';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Indtast et gyldigt telefonnummer';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sessionen er udløbet. Indlæs siden igen, og prøv igen.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Noget gik galt. Prøv igen senere.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Forbindelsesfejl. Tjek internetforbindelsen, og prøv igen.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
