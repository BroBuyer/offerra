...
Teal handelsplatform-skabelon til Offerra, visuel klon af landingen (hjem, undersider, CSS, billeder, chat).

- **Kildesprog:** engelsk i roden (`SITE_LANG=en`); dansk i `langs/da/`; Spanish in `langs/es/`; Norwegian in `langs/no/`; Dutch in `langs/nl/`; Portuguese in `langs/pt/`; Danish in `langs/da/`; Malay in `langs/ms/`
