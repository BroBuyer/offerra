...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Unesite valjani broj telefona',
  1: L.valPhoneCountry || 'Nevažeći kod zemlje',
  2: L.valPhoneShort || 'Broj telefona je prekratak',
  3: L.valPhoneLong || 'Broj telefona je predugačak',
  4: L.valPhoneInvalid || 'Unesite valjani broj telefona',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Unesite broj telefona';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Unesite valjani broj telefona';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sesija je istekla. Osvježite stranicu i pokušajte ponovno.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Nešto je pošlo po krivu. Pokušajte ponovno kasnije.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Pogreška veze. Provjerite internet i pokušajte ponovno.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
