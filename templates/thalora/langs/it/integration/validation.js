...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Inserisci un numero di telefono valido',
  1: L.valPhoneCountry || 'Prefisso paese non valido',
  2: L.valPhoneShort || 'Il numero di telefono è troppo corto',
  3: L.valPhoneLong || 'Il numero di telefono è troppo lungo',
  4: L.valPhoneInvalid || 'Inserisci un numero di telefono valido',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Inserisci il tuo numero di telefono';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Inserisci un numero di telefono valido';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sessione scaduta. Ricarica la pagina e riprova.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Si è verificato un errore. Riprova più tardi.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Errore di connessione. Controlla la connessione internet e riprova.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
