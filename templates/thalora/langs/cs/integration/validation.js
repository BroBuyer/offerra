...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Zadejte telefonní číslo';
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Relace vypršela. Obnovte stránku a zkuste to znovu.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Něco se pokazilo. Zkuste to později znovu.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Chyba připojení. Zkontrolujte internet a zkuste to znovu.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
