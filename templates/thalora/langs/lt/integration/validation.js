...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Įveskite galiojantį telefono numerį',
  1: L.valPhoneCountry || 'Netinkamas šalies kodas',
  2: L.valPhoneShort || 'Telefono numeris per trumpas',
  3: L.valPhoneLong || 'Telefono numeris per ilgas',
  4: L.valPhoneInvalid || 'Įveskite galiojantį telefono numerį',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Įveskite savo telefono numerį';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Įveskite galiojantį telefono numerį';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sesija baigėsi. Įkelkite puslapį iš naujo ir bandykite dar kartą.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Kažkas nutiko. Bandykite dar kartą vėliau.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Ryšio klaida. Patikrinkite interneto ryšį ir bandykite dar kartą.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
