...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Zadajte telefónne číslo';
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Relácia vypršala. Obnovte stránku a skúste to znova.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Niečo sa pokazilo. Skúste to neskôr znova.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Chyba pripojenia. Skontrolujte internet a skúste to znova.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
