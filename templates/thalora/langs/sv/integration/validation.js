...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Ange ett giltigt telefonnummer',
  1: L.valPhoneCountry || 'Ogiltig landskod',
  2: L.valPhoneShort || 'Telefonnumret är för kort',
  3: L.valPhoneLong || 'Telefonnumret är för långt',
  4: L.valPhoneInvalid || 'Ange ett giltigt telefonnummer',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Ange ditt telefonnummer';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Ange ett giltigt telefonnummer';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sessionen har gått ut. Ladda om sidan och försök igen.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Något gick fel. Försök igen senare.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Anslutningsfel. Kontrollera din internetanslutning och försök igen.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
