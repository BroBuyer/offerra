...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Εισαγάγετε έγκυρο αριθμό τηλεφώνου',
  1: L.valPhoneCountry || 'Μη έγκυρος κωδικός χώρας',
  2: L.valPhoneShort || 'Ο αριθμός τηλεφώνου είναι πολύ σύντομος',
  3: L.valPhoneLong || 'Ο αριθμός τηλεφώνου είναι πολύ μακρύς',
  4: L.valPhoneInvalid || 'Εισαγάγετε έγκυρο αριθμό τηλεφώνου',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Εισαγάγετε τον αριθμό τηλεφώνου σας';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Εισαγάγετε έγκυρο αριθμό τηλεφώνου';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Η συνεδρία έληξε. Φορτώστε ξανά τη σελίδα και δοκιμάστε ξανά.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Κάτι πήγε στραβά. Δοκιμάστε ξανά αργότερα.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Σφάλμα σύνδεσης. Ελέγξτε τη σύνδεσή σας στο διαδίκτυο και δοκιμάστε ξανά.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
