...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Introduz um número de telefone válido',
  1: L.valPhoneCountry || 'Código de país inválido',
  2: L.valPhoneShort || 'O número de telefone é demasiado curto',
  3: L.valPhoneLong || 'O número de telefone é demasiado longo',
  4: L.valPhoneInvalid || 'Introduz um número de telefone válido',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Introduz o teu número de telefone';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Introduz um número de telefone válido';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'A sessão expirou. Recarrega a página e tenta novamente.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Algo correu mal. Tenta novamente mais tarde.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Erro de ligação. Verifica a tua ligação à internet e tenta novamente.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
