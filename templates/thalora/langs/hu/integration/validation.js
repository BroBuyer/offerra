...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Adjon meg egy érvényes telefonszámot',
  ...
  4: L.valPhoneInvalid || 'Adjon meg egy érvényes telefonszámot',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Adja meg a telefonszámát';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Adjon meg egy érvényes telefonszámot';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'A munkamenet lejárt. Frissítse az oldalt, és próbálja újra.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Valami hiba történt. Próbálja újra később.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Kapcsolódási hiba. Ellenőrizze az internetet, és próbálja újra.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
