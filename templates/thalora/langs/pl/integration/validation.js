...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Podaj prawidłowy numer telefonu',
  1: L.valPhoneCountry || 'Nieprawidłowy kod kraju',
  2: L.valPhoneShort || 'Numer telefonu jest za krótki',
  3: L.valPhoneLong || 'Numer telefonu jest za długi',
  4: L.valPhoneInvalid || 'Podaj prawidłowy numer telefonu',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Podaj numer telefonu';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Podaj prawidłowy numer telefonu';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sesja wygasła. Odśwież stronę i spróbuj ponownie.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Coś poszło nie tak. Spróbuj ponownie później.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Błąd połączenia. Sprawdź internet i spróbuj ponownie.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
