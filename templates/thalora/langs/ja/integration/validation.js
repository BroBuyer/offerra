...
const phoneErrorMap = {
  0: L.valPhoneInvalid || '有効な電話番号を入力してください',
  1: L.valPhoneCountry || '国番号が無効です',
  2: L.valPhoneShort || '電話番号が短すぎます',
  3: L.valPhoneLong || '電話番号が長すぎます',
  4: L.valPhoneInvalid || '有効な電話番号を入力してください',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || '電話番号を入力してください';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || '有効な電話番号を入力してください';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'セッションの有効期限が切れました。ページを再読み込みして、もう一度お試しください。');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || '問題が発生しました。しばらくしてからもう一度お試しください。');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || '接続エラーです。インターネット接続を確認して、もう一度お試しください。');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
