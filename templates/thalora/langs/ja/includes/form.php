...
$form_submit = $form_submit ?? 'アカウントを作成';
...
>
  <div class="form-already-registered hidden" data-already-registered>
    ...
    <p class="form-already-registered__text">
      お客様のリクエスト： <?= e(SITE_NAME) ?> を受け付けました。担当者からのお電話をお待ちください。
    </p>
  </div>
...
<?php endif; ?>
    ...
    <div class="apx-lead-row lead-form-row">
      <label class="apx-lead-field field" data-field="fname">
        ...
        <input class="input" type="text" name="fname" placeholder="名" required pattern="[^\d]+" autocomplete="given-name" minlength="2" maxlength="50" inputmode="text" autocapitalize="words" spellcheck="false" title="文字のみご入力ください（数字・記号は使用できません）">
    ...
    <div class="apx-lead-row lead-form-row">
      <label class="apx-lead-field field" data-field="lname">
        ...
        <input class="input" type="text" name="lname" placeholder="姓" required pattern="[^\d]+" autocomplete="family-name" minlength="2" maxlength="50" inputmode="text" autocapitalize="words" spellcheck="false" title="文字のみご入力ください（数字・記号は使用できません）">
        <span class="field-error hide" role="alert" aria-live="polite"></span>
      </label>
    </div>
    ...
    <div class="apx-lead-badges" aria-hidden="true">
      ...
      <img src="<?= asset('static/img/svg/payment-4.svg') ?>" alt="安全な決済">
    </div>
  </div>
</form>
</div>
