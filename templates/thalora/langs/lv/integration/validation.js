...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Ievadiet derīgu tālruņa numuru',
  1: L.valPhoneCountry || 'Nederīgs valsts kods',
  2: L.valPhoneShort || 'Tālruņa numurs ir pārāk īss',
  3: L.valPhoneLong || 'Tālruņa numurs ir pārāk garš',
  4: L.valPhoneInvalid || 'Ievadiet derīgu tālruņa numuru',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Ievadiet tālruņa numuru';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Ievadiet derīgu tālruņa numuru';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sesija beigusies. Lūdzu, pārlādējiet lapu un mēģiniet vēlreiz.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Kaut kas nogāja greizi. Lūdzu, mēģiniet vēlāk.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Savienojuma kļūda. Pārbaudiet interneta savienojumu un mēģiniet vēlreiz.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
