...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Voer een geldig telefoonnummer in',
  1: L.valPhoneCountry || 'Ongeldige landcode',
  2: L.valPhoneShort || 'Het telefoonnummer is te kort',
  3: L.valPhoneLong || 'Het telefoonnummer is te lang',
  4: L.valPhoneInvalid || 'Voer een geldig telefoonnummer in',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Voer je telefoonnummer in';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Voer een geldig telefoonnummer in';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Sessie verlopen. Laad de pagina opnieuw en probeer het nog eens.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Er ging iets mis. Probeer het later opnieuw.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Verbindingsfout. Controleer je internetverbinding en probeer het opnieuw.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
