...
const phoneErrorMap = {
  0: L.valPhoneInvalid || 'Introduce un número de teléfono válido',
  1: L.valPhoneCountry || 'Código de país no válido',
  2: L.valPhoneShort || 'El número de teléfono es demasiado corto',
  3: L.valPhoneLong || 'El número de teléfono es demasiado largo',
  4: L.valPhoneInvalid || 'Introduce un número de teléfono válido',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Introduce tu número de teléfono';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || L.valPhoneInvalid || 'Introduce un número de teléfono válido';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'La sesión ha caducado. Recarga la página e inténtalo de nuevo.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Algo ha salido mal. Inténtalo de nuevo más tarde.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Error de conexión. Comprueba tu conexión a internet e inténtalo de nuevo.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
