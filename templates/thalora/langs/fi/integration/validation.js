...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return L.valPhoneEmpty || 'Syötä puhelinnumero';
...
function setupFormValidation(form) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!tokenOk) {
      showFormMessage(form, L.valSessionExpired || 'Istunto vanhentui. Lataa sivu uudelleen ja yritä uudelleen.');
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || L.valGenericError || 'Jokin meni pieleen. Yritä myöhemmin uudelleen.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, L.valConnectionError || 'Yhteysvirhe. Tarkista internetyhteys ja yritä uudelleen.');
    } finally {
      preloader?.classList.add('hidden');
    }
  });
