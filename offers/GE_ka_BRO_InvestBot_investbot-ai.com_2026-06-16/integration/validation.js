...
const phoneErrorMap = {
  0: 'შეიყვანეთ სწორი ტელეფონის ნომერი',
  1: 'ქვეყნის კოდი არასწორია',
  2: 'ტელეფონის ნომერი ძალიან მოკლეა',
  3: 'ტელეფონის ნომერი ძალიან გრძელია',
  4: 'შეიყვანეთ სწორი ტელეფონის ნომერი',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'შეიყვანეთ თქვენი ტელეფონის ნომერი';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'შეიყვანეთ სწორი ტელეფონის ნომერი';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  const phoneCountry = form.querySelector('input[name="phone_country"]')?.value || 'ge';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'რაღაც შეცდომა მოხდა. სცადეთ მოგვიანებით.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'კავშირის შეცდომა. შეამოწმეთ ინტერნეტი და სცადეთ ხელახლა.');
    } finally {
