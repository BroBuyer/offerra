...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Telefon numaranızı girin';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Geçerli bir telefon numarası girin';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  const phoneCountry = form.querySelector('input[name="phone_country"]')?.value || 'tr';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Bir şeyler ters gitti. Lütfen daha sonra tekrar deneyin.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Bağlantı hatası. İnternet bağlantınızı kontrol edip tekrar deneyin.');
    } finally {
