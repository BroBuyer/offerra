...
function setupFormValidation(form) {
  ...
  const phoneCountry = form.querySelector('input[name="phone_country"]')?.value || 'ch';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Etwas ist schiefgelaufen. Bitte versuchen Sie es später erneut.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Verbindungsfehler. Bitte überprüfen Sie Ihre Internetverbindung und versuchen Sie es erneut.');
    } finally {
