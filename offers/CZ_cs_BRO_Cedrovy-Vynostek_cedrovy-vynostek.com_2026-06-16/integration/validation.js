...
const phoneErrorMap = {
  0: 'Zadejte platné telefonní číslo',
  1: 'Neplatná předvolba země',
  2: 'Telefonní číslo je příliš krátké',
  3: 'Telefonní číslo je příliš dlouhé',
  4: 'Zadejte platné telefonní číslo',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Zadejte své telefonní číslo';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Zadejte platné telefonní číslo';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  const phoneCountry = form.querySelector('input[name="phone_country"]')?.value || 'cz';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Něco se pokazilo. Zkuste to prosím později.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Chyba připojení. Zkontrolujte prosím internetové připojení a zkuste to znovu.');
    } finally {
