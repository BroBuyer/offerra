...
const phoneErrorMap = {
  0: 'Zadajte platné telefónne číslo',
  1: 'Neplatná predvoľba krajiny',
  2: 'Telefónne číslo je príliš krátke',
  3: 'Telefónne číslo je príliš dlhé',
  4: 'Zadajte platné telefónne číslo',
};
...
/** Meno, priezvisko, email — štandardná validácia prehliadača. */
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Zadajte svoje telefónne číslo';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Zadajte platné telefónne číslo';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  const phoneCountry = form.querySelector('input[name="phone_country"]')?.value || 'sk';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const language = form.querySelector('input[name="language"]')?.value || 'sk';
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Niečo sa pokazilo. Skúste to znova neskôr.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Chyba pripojenia. Skontrolujte internetové pripojenie a skúste znova.');
    } finally {
