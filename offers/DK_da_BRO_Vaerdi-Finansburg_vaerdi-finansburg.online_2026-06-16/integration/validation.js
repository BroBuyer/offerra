...
const phoneErrorMap = {
  0: 'Indtast et gyldigt telefonnummer',
  1: 'Ugyldigt landekode-præfiks',
  2: 'Telefonnummeret er for kort',
  3: 'Telefonnummeret er for langt',
  4: 'Indtast et gyldigt telefonnummer',
};
...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Indtast dit telefonnummer';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Indtast et gyldigt telefonnummer';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  const phoneCountry = form.querySelector('input[name="phone_country"]')?.value || 'dk';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Noget gik galt. Prøv igen senere.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Forbindelsesfejl. Kontrollér din internetforbindelse og prøv igen.');
    } finally {
