...
function validatePhone(phoneInput, iti) {
  ...
  if (!trimmed) return 'Ingresá tu número de teléfono';
  if (!iti.isValidNumber()) {
    const code = iti.getValidationError();
    return phoneErrorMap[code] || 'Ingresá un número de teléfono válido';
  }
  return '';
}
...
function setupFormValidation(form) {
  ...
  const phoneCountry = form.querySelector('input[name="phone_country"]')?.value || 'ar';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!data.ok) {
        showFormMessage(form, data.error || 'Algo salió mal. Intentá de nuevo más tarde.');
        return;
      }
      ...
      console.error(err);
      showFormMessage(form, 'Error de conexión. Verificá tu conexión a Internet e intentá de nuevo.');
    } finally {
