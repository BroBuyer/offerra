    ...
    public function handle(OfferScanner $scanner, TemplateCatalog $catalog): int
    {
        ...
        foreach ($scanner->scan() as $row) {
            ...
            Offer::create([
                ...
                'phone' => strtolower($manifest['phone'] ?? $row['phone'] ?? ''),
                'min_deposit' => (string) ($manifest['min_deposit'] ?? $row['min_deposit'] ?? '250'),
                'currency' => strtoupper((string) ($manifest['currency'] ?? $row['currency'] ?? 'EUR')),
